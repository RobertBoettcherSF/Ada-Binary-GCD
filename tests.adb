--  Standalone test suite for Binary_GCD (main program).
...
begin
   Ada.Text_IO.Put_Line ("Binary_GCD tests");
   ...
   begin
      ...
      --  Spot-check a few cells as individual PASSes for the log.
   ...
   begin
      ...
      Check (Gcd_Recursive (Big1, Big2) = Gcd (Big1, Big2),
             "rec large powers");
      --  Keep recursive depth moderate for the huge odd case:
      Check (Gcd (U (2 ** 40), U (2 ** 30 + 7)) =
             Gcd_Euclidean (U (2 ** 40), U (2 ** 30 + 7)),
             "stein=euclid 2^40 vs 2^30+7");
      Check (Gcd (U (2 ** 40), U (2 ** 30 + 7)) =
             Gcd_Recursive (U (2 ** 40), U (2 ** 30 + 7)),
             "rec 2^40 vs 2^30+7");
   end;
